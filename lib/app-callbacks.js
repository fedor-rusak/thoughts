...
const lineListener = function (currentBackend, appState, line) {
	...
	if (mode === "command") {
		...
		if (normalized === 'clear') {
			...
			let handleBundledData = (err, data) => {
				if (err) {
			...
			render(terminalSize, 
				[
					"Here should be:",
					" * columns of existing tags",
					[" * list of ", {color:"green", text:"chosen"}, " tags"],
					" * first note that fits chosen tags or comment that nothing was found",
					"",
					"Input allowed: ",
					" * arrows to navigate over tags",
					[" * space to ", {color:"green", text:"toggle"}, " tag"],
					" * enter to chose note and go into browse mode",
					" * ESC or q to exit search mode"
				],
				mutableOutput);
			mutableOutput.mute();
