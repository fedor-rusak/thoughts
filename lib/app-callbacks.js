...
const lineListener = function (currentBackend, appState, line) {
    ...
    if (mode === 'command') {
        ...
        if (normalized === 'clear' || normalized === 'очистить') {
            appState.commands = [];
        ...
        else if (normalized === 'content' || normalized === 'содержание') {
            appState.mode = 'content';
            toPrint = currentNoteState.content || '';
            asUser = true;
            result = 'mode = content';
        }
        else if (normalized === 'tags' || normalized === 'тэги') {
            appState.mode = 'tags';
            toPrint = currentNoteState.tags || '';
            asUser = true;
            result = 'mode = tags';
        }
        else if (normalized === 'now' || normalized === 'сейчас') {
            currentNoteState.date = new Date().toISOString();
            result = 'date set';
        }
        else if (normalized === 'state' || normalized === 'состояние') {
            toPrint = JSON.stringify(currentNoteState, null, 4).replace(/\n/g, '\r\n') + '\r\n';
        }
        else if (normalized === 'push' || normalized === 'добавить') {
        ...
        else if (normalized === 'records' || normalized === 'записи') {
        ...
        else if (normalized === 'thoughts' || normalized === 'мысли') {
            if (appState.records.length === 0) {
                mutableOutput.write('No thoughts to remember now.  mode = command\r\n')
            }
            else {
        ...
        else if (normalized === 'save' || normalized === 'сохранить') {
        ...
        else if (normalized === 'load' || normalized === 'загрузить') {
        ...
        else if (normalized === 'new' || normalized === 'создать') {
            appState.currentNoteState = {};
            appState.noteIndex = -1;
            result = 'State clear. mode = command'
        }
        else if (normalized === 'drop'  || normalized === 'удалить') {
        ...
        else if (normalized === 'browse'  || normalized === 'смотреть') {
            if (appState.records.length === 0) {
        ...
        else if (normalized === 'help' || normalized === 'помощь') {
        ...
        else if (normalized === 'development-thoughts' || normalized === 'идеи-по-разработке') {
            appState.mode = 'loading';
            result = 'mode = loading';
            waitingCommandComment(mutableOutput, line, result);
        ...
        else if (normalized === 'gist-token' || normalized === 'гист-токен') {
            appState.mode = 'gist-token';
            result = 'No typing is shown. mode = gist-token'
            waitingCommandComment(mutableOutput, line, result);
        }
        else if (normalized === 'gist-name' || normalized === 'гист-название') {
            appState.mode = 'gist-name';
            result = 'mode = gist-name'
        }
        else if (normalized === 'use-gist' || normalized === 'использовать-гист') {
            try {
        ...
        else if (normalized === 'use-fs' || normalized === 'использовать-фс') {
            try {
        ...
        else if (normalized === 'search' || normalized === 'искать') {
            if (appState.records.length === 0) {
        ...
        else if (normalized === 'forget' || normalized === 'забудь') {
            commands.push(line);
...
const keyListener = function (currentBackend, appState, exitCallback, s, key) {
    ...
    if (mode === 'records') {
        if (key.name === 'escape' || key.name === 'q' || key.name === 'й') {
            backToCommands(currentBackend, appState);
        }
