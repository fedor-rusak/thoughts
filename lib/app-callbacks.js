...
const lineListener = function (currentBackend, appState, line) {
	...
	if (mode === "command") {
		...
		if (normalized === 'clear') {
			...
			toPrint = JSON.stringify(currentNoteState, null, 4).replaceAll("\n","\r\n")+"\r\n";
		}
			...
			currentBackend.fs.writeFileSync(dataFilePath, JSON.stringify(appState.records, null, 4));
			result = "Success"
		}
		else if (normalized === "load") {
			try {
				appState.records = JSON.parse(currentBackend.fs.readFileSync(dataFilePath));
				result = "Success"
			}
			catch (e) {
				result = "Failed";
			...
			try {
				appState.records = JSON.parse(currentBackend.fs.readFileSync("./lib/development-thoughts.json"));
